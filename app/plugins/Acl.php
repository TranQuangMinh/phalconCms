...
class Acl extends Plugin
{
    ...
    private function getRole()
    {
        ...
        if (!$auth) {
            ...
            if ($auth->admin_session == true) {
                $role = 'admin'; // Check role user đã đăngn hập
            } else {
                $role = 'member';
            }
        }
    ...
    private function accessDenied($role, $resourceKey = null, $resourceVal = null, View $view)
    {
        ...
        $view->setPartialsDir('');
        $view->e = "$role - Access Denied to resource <b>$resourceKey::$resourceVal</b>";
    ...
    private function resourceNotFound($resourceKey, View $view)
    {
        ...
        $view->e = "Acl resource <b>$resourceKey</b> in <b>/app/config/acl.php</b> not exists";
